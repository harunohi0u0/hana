...
const定位= document.getElementById('fx-day');
if(定位) { for(let i=1;i<=31;i++) 定位.add(new Option(`매월 ${i}일`, i)); }
...
window.calcClosePnL = function() {
    ...
    const pnl = (ticks * spec.tickValue * closeQty) - comm; 
...
window.processCloseJournal = async function(e) {
    ...
    const j = localJournals.find(x => x.id === id);
    const originalGroupId = j.groupId || j.id; 
    ...
    if (closeQty === maxQty || j.status === 'closed') {
        ...
        delete closedEntry.id; 

        const remainQty = maxQty - closeQty;
        j.qty = remainQty;
        j.groupId = originalGroupId; 
...
window.addJournal = async function(e) {
    ...
    const entry = {
        groupId: oldJ ? (oldJ.groupId || oldJ.id) : 'grp_' + Date.now(), 
...
window.renderJournals = function() {
    ...
    filteredJournals.forEach(j => {
        if(j.status === 'closed') {
            ...
            if (j.slPrice && risk > 0) {
                ...
                const actReward = j.position === 'long' ? (j.closePrice - j.entryPrice) : (g.entryPrice - j.closePrice);
    ...
    totalClosed = Object.keys(groupPnL).length; 
    wins = Object.values(groupPnL).filter(pnl => pnl > 0).length; 
            ...
            <div class="relative group/sub flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 rounded-xl border mb-2 last:mb-0 ${j.status==='closed' ? 'bg-[#F0FDFA] border-pancake-primary/30' : 'bg-gray-50 border-gray-200'}">
                ...
                <div class="flex justify-between sm:justify-end items-center gap-3 w-full sm:w-auto border-t sm:border-0 border-gray-100 pt-2 sm:pt-0">
                    ${j.status === 'closed' ? `
                        ...
                        <span class="font-brand font-bold text-base ${j.pnl > 0 ? 'text-pancake-success' : 'text-pancake-failure'}">${j.pnl > 0 ? '+' : ''}$${formatNumber(parseFloat(j.pnl).toFixed(2))}</span>
                    ` : `
